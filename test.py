...
def calendario(url):
    ...
    cabecalho = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.76 Safari/537.36'}  # cabecalho para obter a requisicao do site (site só aceita acesso por navegador(simulação))
    ...
    for tr in linhas:
        ...
        horario = arrow.get(horario,
                            'YYYY/MM/DD HH:mm:ss')  # converter uma string de horario em um formato aceito pelo python
        """
        ...
        touro = impacto.findAll('i', {
            'class': 'grayFullBullishIcon'})  # separando o impacto da noticia pela tag html 'grayFullBullishIcon' e sua quantidade respectiva
        ...
        calendario.append('{}{}'.format(url, a[
            'href']))  # separando a url da noticia com o url do site e tag de referencia html 'href'
...
def enviarMensagens(msgID, texto, botao=''):
    ...
    bot.sendMessage(msgID, texto, reply_markup=botao,
                    disable_notification=False)  # retorna uma mensagem pelo ID da conversa + um texto + um botao
...
saber mais sobre o meu funcionamento ou quiser relatar alguma coisa, entre em contato com o meu desenvolvedor, é só clicar no botão abaixo :backhand_index_pointing_down:',
                                  use_aliases=True))  # msg dando boas vindas e explicando o funcionamento do bot
        botao = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(
            text=(emoji.emojize('+ INFO :mobile_phone_with_arrow:', use_aliases=True)),
            url='https://t.me/alpdias')]])  # botao com link para ajuda
        enviarMensagens(msgID, bemvindo, botao)

    elif msg['text'] == '/start' and msgID == usuario:
        solicitar = (emoji.emojize(f'Olá {nome}, o que deseja fazer? :thinking_face:',
                                   use_aliases=True))  # msg dando boas vindas e solicitando uma açao
        botao = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(
            text=(emoji.emojize('ATUALIZAR :globe_with_meridians:', use_aliases=True)), callback_data='atualizar')], [
                                                          InlineKeyboardButton(text=(
                                                              emoji.emojize('AGENDAR :timer_clock:', use_aliases=True)),
                                                                               callback_data='agendar')], [
                                                          InlineKeyboardButton(text=(
                                                              emoji.emojize('LOOP :counterclockwise_arrows_button:',
                                                                            use_aliases=True)),
                                                                               callback_data='loop')]])  # botoes par atualizaçao
...
def responderMensagens(msg):
    ...
    msgID, respostaID, resposta = telepot.glance(msg,
                                                 flavor='callback_query')  # variaveis que recebem o 'callback query' da resposta (necessario 3 variaveis, o ID da conversa e o da resposta sao diferentes)

    if resposta == 'atualizar':
        try:
            bot.answerCallbackQuery(msgID, text=(emoji.emojize('Atualizando... :globe_with_meridians:',
                                                               use_aliases=True)))  # mostra um texto/alerta na tela do chat
            sleep(2)
...
\nPara ver mais acesse :backhand_index_pointing_down:', use_aliases=True)).strip()  # noticia formatada
                ...
                botao = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(
                    text=(emoji.emojize('+ NOTÍCIA :receipt:', use_aliases=True)),
                    url=f'{link.strip()}')]])  # botao com link acesso a noticia
                ...
                if verificacao == 0 and atual == 0:
                    ...
                    for item in range(0, 6):
                        del dados[
                            0]  # apaga as ultimas informaçoes ja usadas(6 primeiros itens na lista), para nao ter repetiçoes
                ...
                if quantidade == 0:
                    terminado = (emoji.emojize(
                        'Todas as notícias já foram enviadas, processo finalizado :beaming_face_with_smiling_eyes::thumbs_up:',
                        use_aliases=True))
            ...
            desatualizado = (emoji.emojize('Erro inesperado ao atualizar! :pensive_face:',
                                           use_aliases=True))  # msg de erro na atualizaçao
        ...
        sleep(agendar)
        atualizando = (
            emoji.emojize('Atualizando... :globe_with_meridians:', use_aliases=True))  # msg de atualizaçao dos dados
...
\nPara ver mais acesse :backhand_index_pointing_down:', use_aliases=True)).strip()  # noticia formatada
        ...
        while True:
            ...
            if quantidade == 0:
                ...
                agendar = (agendar * 60) + 5
                sleep(agendar)
                atualizando = (emoji.emojize('Atualizando... :globe_with_meridians:',
                                             use_aliases=True))  # msg de atualizaçao dos dados
                enviarMensagens(respostaID, atualizando)
                ...
                try:
                    ...
                    while True:  # mostra o horario atual da maquina para verificar junto com o horario da noticia (adicionado correçao de fuso horario)
                        ...
                        if local == 'Argentina':
                            ...
                            bandeira = (
                                emoji.emojize('\U0001F3F4\U000E0067\U000E0062\U000E0065\U000E006E\U000E0067\U000E007F',
                                              use_aliases=True))
                        ...
                        impacto = dados[3]  # dado especifico para o impacto da noticia
                        impacto = (emoji.emojize(int(impacto) * f':cow_face:',
                                                 use_aliases=True))  # transformando dado em emoji
                        link = dados[4]  # dado especifico para o link da noticia
                        chamada = dados[5]  # dado especifico para a chamada da noticia
